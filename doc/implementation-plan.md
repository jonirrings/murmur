...
```
m.o0x0o.com 流量 → Cloudflare Edge 按以下优先级分发:
...
② Pages 自定义域名（兜底，最低优先级）:
    /*           → murmur-solid (TanStack Start)

未匹配任何 Worker Route 的请求自动 fall through 到 Pages。
无需代理代码，Cloudflare 边缘路由原生完成分发。
```
...
> **与反向代理方案的对比**：不再需要 `app.all("/admin/*", ...)` 和 `app.all("/*", ...)` 代理中间件。Cloudflare 边缘路由自动将 `/admin/*` 发给 admin Worker，`/*` 发给 Pages。
...
## Phase 3: Solid 前台 — murmur-solid Pages（预计 3-4h）

### 步骤 3.1：脚手架 TanStack Start (Solid) 项目

```bash
mkdir -p apps/solid && cd apps/solid && pnpm init
...
### 步骤 3.2：定义 TanStack Router 文件路由

使用 `createFileRoute` from `@tanstack/solid-router`：

```
apps/solid/src/routes/
    ...
    <div class="min-h-screen bg-white dark:bg-gray-950">
      <header class="border-b px-4 py-3">
        <nav class="mx-auto flex max-w-2xl items-center justify-between">
          <Link to="/" class="text-lg font-bold">murmur / 碎碎念</Link>
...
function HomePage() {
  ...
  const query = createQuery(() => ({
    ...
    queryFn: () => fetch(
      `/api/public/snippets?page=${search().page}&q=${search().q}&tag=${search().tag}`
    ).then(r => r.json()) as Promise<PaginatedResponse<SnippetListItem>>,
  }));
  // 渲染碎片列表卡片 + 分页
}
```
...
function PostPage() {
  ...
  const query = createQuery(() => ({
    ...
    queryFn: () => fetch(`/api/public/snippets/${params().slug}`)
      .then(r => r.json()) as Promise<SnippetDetail>,
  }));
...
### 步骤 3.3：MurmurContent 组件

`apps/solid/src/components/MurmurContent.tsx` — 使用 `solid-markdown` + `rehype-shiki`：
...
### 步骤 3.6：部署 murmur-solid 到 Cloudflare Pages
...
# 绑定自定义域名（Dashboard 或 CLI）
# Pages → murmur-solid → Custom domains → m.o0x0o.com
```
...
- [ ] Worker Routes 已配置（api + admin）
- [ ] Pages 自定义域名绑定 `m.o0x0o.com`
...
└── /*             → murmur-solid (TanStack Start, Cloudflare Pages)

Cloudflare Worker Routes take priority over Pages custom domain.
...
Phase 0 (基础设施)
  └── Phase 1 (murmur-api Worker)
        ...
        └── Phase 3 (murmur-solid Pages)
              └── Phase 4 (E2E 测试 + 上线)
```

- Phase 1 完成后，Phase 2 和 Phase 3 **可以并行开发**
- Phase 3 部署后立即验证路由共存（步骤 3.6）
...
Phase 1 (murmur-api Worker):
  ...
  murmur/apps/api/src/__tests__/snippets.test.ts
  murmur/apps/api/src/__tests__/tags.test.ts
  murmur/apps/api/src/__tests__/auth.test.ts
  murmur/apps/api/src/__tests__/rate-limit.test.ts
...
Phase 2 (murmur-admin Worker):
  ...
  murmur/apps/admin/src/components/__tests__/editor.test.tsx
  murmur/.github/workflows/deploy-admin.yml

Phase 3 (murmur-solid Pages):
  murmur/apps/solid/package.json
  murmur/apps/solid/tsconfig.json
  murmur/apps/solid/app.config.ts
  murmur/apps/solid/src/routes/__root.tsx
  murmur/apps/solid/src/routes/index.tsx
  murmur/apps/solid/src/routes/post.$slug.tsx
  murmur/apps/solid/src/routes/tags.tsx
  murmur/apps/solid/src/components/MurmurContent.tsx
  murmur/apps/solid/src/components/SnippetCard.tsx
  murmur/apps/solid/src/components/SearchBar.tsx
  murmur/apps/solid/src/components/__tests__/MurmurContent.test.tsx
  murmur/.github/workflows/deploy-solid.yml
...
| 方面 | 原反向代理方案 | 当前 Worker Routes 方案 |
|------|-------------|----------------------|
| 入口 | Worker 接管全部流量然后 fetch Pages | 边缘路由直接分发到各自目的地 |
| 额外延迟 | +1 跳（Worker → Pages） | 0 |
| 代理代码 | 需要维护 `app.all("/admin/*", ...)` 和 `app.all("/*", ...)` | 零代理代码 |
| 配置复杂度 | 需要 env vars 传 Pages URL | Worker Routes 在 wrangler.jsonc 中声明 |
| Worker 额度消耗 | 前台每个请求消耗 1 次 Worker | 前台请求不经过 Worker，直接到 Pages |
| 路由控制 | 代码层（Hono 路由） | 基础设施层（Cloudflare 边缘） |
| 部署独立性 | Worker 发新版需确保代理不变 | 三个单元完全独立部署 |
...
| 8 | Worker Routes + Pages 共存 | `/admin` → Worker, `/post/x` → Pages | 3 |
